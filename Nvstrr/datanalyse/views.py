...
class UserPortsListView(ListView):
    ...
    template_name = 'datanalyse/letsee.html'
    ...
    context_object_name = "userports_list"
    ...
    def get_queryset(self):
        ...
        if query:
            ...
            result = result.filter(
                reduce(operator.and_, (Q(companies__icontains=q) for q in query_list)))
...
class DetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        url = link+"%s:%s" % ("BOM", allu)
        linkgraph = "https://chart.finance.yahoo.com/t?s=%s.BO&lang=en-IN&region=IN&width=500&height=300" % (
            allu)
        ...
        try:
            for urls in search(allu, tld='co.in', lang='en', stop=20):
                if 'wikipedia' in urls:
                    ...
                    context["summary"] = (wikipedia.summary(
                        page)).encode('ascii', 'ignore')
        ...
        rsslink = 'https://www.google.co.in/finance/company_news?q=BOM:%s&output=rss' % (
            allu)
